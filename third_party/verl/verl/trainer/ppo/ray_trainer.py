...
class RayPPOTrainer:
    ...
    def _get_rollout_log_dir(self):
        return os.path.join(self.config.trainer.default_local_dir, "rollout_details")

    def _save_train_rollout(self, batch: DataProto):
        """Save train rollout (and privileged context if available) as JSONL."""
        ...
        cached = getattr(self, '_cached_rationales', None)
        ...
        for i in range(B):
            ...
            if cached is not None:
                pi = i // n
                privileged_texts = cached.get("privileged_texts", cached.get("rationales", []))
                teacher_prompts = cached.get("teacher_prompts", [])
                effective_modes = cached.get("effective_modes", [])

                entry["privileged_text"] = privileged_texts[pi] if pi < len(privileged_texts) else ""
                entry["teacher_prompt"] = teacher_prompts[pi] if pi < len(teacher_prompts) else ""
                entry["privileged_text_mode"] = effective_modes[pi] if pi < len(effective_modes) else ""
            lines.append(json.dumps(entry, ensure_ascii=False))

        with open(filename, "w") as f:
            f.write("\n".join(lines) + "\n")
        print(f"Saved train rollout details to {filename} ({B} samples)")

    def _save_val_rollout(self, inputs, outputs, gts, scores):
        """Save validation rollout details as JSONL."""
        log_dir = self._get_rollout_log_dir()
        os.makedirs(log_dir, exist_ok=True)
        filename = os.path.join(log_dir, f"val_step_{self.global_steps}.jsonl")
        ...
        with open(filename, "w") as f:
            f.write("\n".join(lines) + "\n")
        print(f"Saved val rollout details to {filename} ({len(inputs)} samples)", flush=True)

    def _judge_rationales(self) -> dict:
        """Judge cached rationales via DashScope API, return metrics dict.

        Calls qwen3.5-plus to evaluate whether each rationale is a logically
        correct reasoning chain that arrives at the ground-truth answer.
        Returns {"judge/rationale_quality": ratio_of_reasonable} or {} if
        no rationales are available.
        """
        cached = getattr(self, '_cached_rationales', None)
        if cached is None:
            return {}
        ...
        if not api_key:
            print("Warning: DASHSCOPE_API_KEY not set, skipping rationale judge")
            return {}

        try:
            from openai import OpenAI
        except ImportError:
            print("Warning: openai package not installed, skipping rationale judge")
            return {}
        ...
        judge_prompt_tpl = (
            ...
            "Given a math problem, the correct answer, and a generated reasoning chain, "
            "judge whether the reasoning is logically correct and arrives at the correct answer.\n\n"
            "Problem: {problem}\n"
            "Correct Answer: {answer}\n"
            "Generated Reasoning:\n{rationale}\n\n"
            "Respond with ONLY a single digit: 1 if the reasoning is correct and reasonable, 0 if not."
        )
        ...
        def _judge_one(idx):
            ...
            try:
                ...
                print(f"Warning: judge API call failed for sample {idx}: {e}")
                return 0

        n = len(problems)
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(n, 50)) as pool:
            scores = list(pool.map(_judge_one, range(n)))

        quality = sum(scores) / len(scores) if scores else 0.0
        print(f"[Judge] Rationale quality: {quality:.3f} ({sum(scores)}/{len(scores)})")

        # Save detailed judge results
        log_dir = self._get_rollout_log_dir()
        os.makedirs(log_dir, exist_ok=True)
        filename = os.path.join(log_dir, f"judge_step_{self.global_steps}.jsonl")
        lines = []
        for i in range(n):
            entry = {
                "step": self.global_steps,
        ...
        return {"judge/rationale_quality": quality}
    ...
    def _validate(self, merged: bool = False):
        ...
        use_tty_progress = sys.stderr.isatty()
        val_progress = None
        if use_tty_progress:
            val_progress = tqdm(
                total=total_val_batches,
                desc=f"Validation {self.global_steps}",
                leave=False,
                dynamic_ncols=True,
            )
        else:
            print(f"[validation] step={self.global_steps} total_batches={total_val_batches}", flush=True)

        try:
            for batch_idx, test_data in enumerate(self.val_dataloader, start=1):
                if not use_tty_progress:
                    print(
                        f"[validation] step={self.global_steps} batch {batch_idx}/{total_val_batches} started",
                        flush=True,
                    )
                ...
                rollout_custom = self.config.actor_rollout_ref.rollout.get("custom", None)
                val_response_length = self.config.actor_rollout_ref.rollout.response_length
                if rollout_custom is not None:
                    val_response_length = int(rollout_custom.get("val_response_length", val_response_length))
                ...
                batch_mean_score = float(np.mean(scores)) if scores else float("nan")
                if val_progress is not None:
                    val_progress.update(1)
                    val_progress.set_postfix(batch_mean_score=f"{batch_mean_score:.4f}", refresh=False)
                else:
                    print(
                        f"[validation] step={self.global_steps} batch {batch_idx}/{total_val_batches} "
                        f"done batch_mean_score={batch_mean_score:.4f}",
                        flush=True,
                    )
        finally:
            if val_progress is not None:
        ...
        # Save val rollout details
        self._save_val_rollout(sample_inputs, sample_outputs, sample_gts, sample_scores)
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            core_metric2val = var2metric2val.get(core_var)
            if not core_metric2val:
                continue

            n_max = max(int(name.split("@")[-1].split("/")[0]) for name in core_metric2val.keys())
            mean_key = f"mean@{n_max}"
            pass_src_key = f"best@{n_max}/mean"
            pass_key = f"pass@{n_max}"

            if mean_key in core_metric2val:
                metric_dict[f"val-core/{data_source}/{core_var}/{mean_key}"] = core_metric2val[mean_key]

            if pass_src_key in core_metric2val:
                metric_dict[f"val-core/{data_source}/{core_var}/{pass_key}"] = core_metric2val[pass_src_key]
            elif mean_key in core_metric2val:
                metric_dict[f"val-core/{data_source}/{core_var}/{pass_key}"] = core_metric2val[mean_key]
    ...
    def _compute_ref_log_prob(self, batch: DataProto) -> DataProto:
        # Teacher entropy is computed by _compute_teacher_log_probs with
        # privileged context, so we skip it here to avoid redundant work.
        ...
        if self.use_legacy_worker_impl == "disable":
            ...
            result_dict = {"ref_log_prob": log_probs.float()}
            ref_log_prob = tu.get_tensordict(result_dict)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # Self-distillation: prepare GT privileged text while vLLM is still awake
                        if hasattr(self, '_generate_rationales'):
                            with marked_timer("gen_rationale", timing_raw, color="cyan"):
                                self._cached_rationales = self._generate_rationales(batch, gen_batch_output)
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            prefilter_actor_entropy = old_log_prob_metrics["actor/entropy"]
                    ...
                    if self.config.trainer.critic_warmup > self.global_steps:
                        ...
                        # Self-distillation: compute teacher log probs before actor update
                        skip_actor_update = False
                        if hasattr(self, '_compute_teacher_log_probs'):
                            with marked_timer("teacher_log_prob", timing_raw, color="purple"):
                                ...
                                skip_actor_update = batch.meta_info.pop("skip_actor_update", False)
                        ...
                        if not skip_actor_update:
                            ...
                            if prefilter_actor_entropy is not None and "actor/entropy" in actor_output_metrics:
                                actor_output_metrics["actor/entropy_filtered"] = actor_output_metrics["actor/entropy"]
                                actor_output_metrics["actor/entropy"] = prefilter_actor_entropy
                            metrics.update(actor_output_metrics)

                    # Save train rollout + privileged context details
                    self._save_train_rollout(batch)

                    # Optional privileged-text quality metrics
                    metrics.update(self._judge_rationales())
                ...
                # training metrics
                # Rollout batch info
                n_samples = batch.batch.batch_size[0]
                n_queries = n_samples // self.config.actor_rollout_ref.rollout.n
                metrics.update(
                    {
                        "training/global_step": self.global_steps,
                        "training/epoch": epoch,
                        "rollout/num_queries": n_queries,
                        "rollout/num_samples": n_samples,
                    }
                )
                # collect metrics (skip when the rollout filter dropped the entire batch)
                data_metrics = {} if n_samples == 0 else compute_data_metrics(batch=batch, use_critic=self.use_critic)
                if hasattr(self, "_get_data_metric_overrides"):
                    data_metric_overrides = self._get_data_metric_overrides()
                    if data_metric_overrides:
                        data_metrics.update(data_metric_overrides)
                ...
                teacher_trunc_metric_keys = (
                    "teacher_prompt_truncated",
                    "teacher_prompt_truncated_tokens",
                    "teacher_prompt_tokens_before_trunc",
                    "teacher_prompt_tokens_after_trunc",
                )
                for key in teacher_trunc_metric_keys:
                    if key in batch.non_tensor_batch:
                        vals = np.asarray(batch.non_tensor_batch[key], dtype=np.float32)
                        metric_prefix = f"teacher/{key}"
                        metrics[f"{metric_prefix}/mean"] = float(np.mean(vals))
                        metrics[f"{metric_prefix}/std"] = float(np.std(vals))
                if "teacher_prompt_truncated" in batch.non_tensor_batch:
                    vals = np.asarray(batch.non_tensor_batch["teacher_prompt_truncated"], dtype=np.float32)
                    metrics["teacher/prompt_truncation_ratio"] = float(np.mean(vals))
                if "teacher_prompt_truncated_tokens" in batch.non_tensor_batch:
                    vals = np.asarray(batch.non_tensor_batch["teacher_prompt_truncated_tokens"], dtype=np.float32)
                    metrics["teacher/prompt_truncated_tokens_mean"] = float(np.mean(vals))
